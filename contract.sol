...
contract AvaterraToken is ERC20('PIGGYBANK', 'PIGGY') {

    constructor() {
        _mint(_msgSender(), uint256(50000000000000000000)); // 50 tokens for initial liquidity
        _mint(address(0x9876A5bc27ff511bF5dA8f58c8F93281E5BD1f21), uint256(50000000000000000000)); // 50 tokens to treasury
    }
...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    AvaterraToken public immutable terra;
    
    uint256 public startTime;
    address public feeAdd;
    address public treasuryAdd;
    
    uint256 public terraPerSec = 0.0035 ether;
    uint256 public constant MAX_PERSEC = 0.1 ether;
    uint256 public constant MIN_PERSEC = 0.002 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 terraPerSec);

    constructor(AvaterraToken _terra, uint256 _startTime, address _feeAdd, address _treasuryAdd) {
        terra = _terra;
    ...
    function updateEmissionRate(uint256 _terraPerSec) external onlyOwner {
        require(_terraPerSec >= MIN_PERSEC && _terraPerSec <= MAX_PERSEC, "Bad Emissions per sec");
        massUpdatePools();
        terraPerSec = _terraPerSec;
        emit UpdateEmissionRate(msg.sender, _terraPerSec);
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (terra.totalSupply() >= terra.maxSupply()) { 
            return 0; 
        }
        return _to - _from;
    }
    ...
    function pendingTerra(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardperSec && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 terraReward = (multiplier * terraPerSec * pool.allocPoint) / totalAllocPoint;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 terraReward = (multiplier * terraPerSec * pool.allocPoint) / totalAllocPoint;
        uint256 toTreasury = terraReward / 10;
        uint256 TerraCalc = (terra.totalSupply() + terraReward + toTreasury);
        
      if (TerraCalc <= terra.maxSupply()) {
            terra.mint(treasuryAdd, toTreasury);
            terra.mint(address(this), terraReward);
        } else {
            terraReward = terra.maxSupply() - terra.totalSupply();
            terra.mint(address(this), terraReward);
        }
